...
class Plugin_Contact extends Plugin 
{
	...
	public function form()
	{
		...
		foreach ($form_meta as $form => $value)
		{
			...
			if ($value['type'] == 'dropdown')
			{
				$parse_data[$form] .= call_user_func('form_'.$value['type'],
														...
														set_value($form),
														'id="contact_'.$form.'" class="form-control"'
													 );
			}
		...
		$output .= '<span class="contact-button">'.form_submit('contact-submit', ucfirst($button),'class="btn btn-primary btn-lg"').'</span>'.PHP_EOL;
		$output .= form_close();
